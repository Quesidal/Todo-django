...
class TaskManager(models.Manager):
    ...
    def week_tasks_for_user(self, user):
        today = datetime.date.today()
        return self.get_queryset().filter(end_time__range=(today, today + datetime.timedelta(days=7)), state=False) | \
               self.get_queryset().filter(end_time__lt=datetime.date.today(), state=False)
